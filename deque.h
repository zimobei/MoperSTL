#ifndef MOPER_STL_DEQUE_H
#define MOPER_STL_DEQUE_H
...
struct __deque_iterator : public MoperSTL::iterator<MoperSTL::random_access_iterator_tag, T> {
...
//    using iterator = typename __deque_iterator<T, T&, T*>;
//	using const_iterator = typename __deque_iterator<T, const T&, const T*>;
...
public:
  // deque 的型别定义
  typedef MoperSTL::allocator<T>                      allocator_type;
  typedef MoperSTL::allocator<T>                      data_allocator;
  typedef MoperSTL::allocator<T*>                     map_allocator;
  ...
  typedef __deque_iterator<T, T&, T*>                iterator;
  typedef __deque_iterator<T, const T&, const T*>    const_iterator;
  typedef MoperSTL::reverse_iterator<iterator>        reverse_iterator;
  typedef MoperSTL::reverse_iterator<const_iterator>  const_reverse_iterator;
...
public:
  ...
  template <class IIter, typename std::enable_if<
      ...
      typename MoperSTL::iterator_traits<IIter>::iterator_category, MoperSTL::input_iterator_tag>
      , int>::type = 0>
  ...
  {
    copy_init(ilist.begin(), ilist.end(), MoperSTL::forward_iterator_tag());
  }

  deque(const deque& rhs)
  {
    copy_init(rhs.begin(), rhs.end(), MoperSTL::forward_iterator_tag());
  }
  deque(deque&& rhs) noexcept
    :begin_(MoperSTL::move(rhs.begin_)),
    end_(MoperSTL::move(rhs.end_)),
    map_(rhs.map_),
...
public:
  ...
  { copy_assign(ilist.begin(), ilist.end(), MoperSTL::forward_iterator_tag{}); }
  ...
  void     push_front(value_type&& value) { emplace_front(MoperSTL::move(value)); }
  void     push_back(value_type&& value)  { emplace_back(MoperSTL::move(value)); }
...
deque<T>& deque<T>::operator=(const deque& rhs)
{
  if (this != &rhs)
  {
    ...
    if (len >= rhs.size())
    {
      erase(MoperSTL::copy(rhs.begin_, rhs.end_, begin_), end_);
    }
    else
    {
      iterator mid = rhs.begin() + static_cast<difference_type>(len);
      MoperSTL::copy(rhs.begin_, mid, begin_);
...
void deque<T>::emplace_front(Args&& ...args)
{
  ...
  {
    data_allocator::construct(begin_.cur - 1, MoperSTL::forward<Args>(args)...);
    --begin_.cur;
  }
  else
  {
    ...
    {
      --begin_;
      data_allocator::construct(begin_.cur, MoperSTL::forward<Args>(args)...);
    }
    catch (...)
    {
      ++begin_;
...
void deque<T>::emplace_back(Args&& ...args)
{
  ...
  {
    data_allocator::construct(end_.cur, MoperSTL::forward<Args>(args)...);
    ++end_.cur;
  }
  else
  {
...
typename deque<T>::iterator deque<T>::emplace(iterator pos, Args&& ...args)
{
  ...
  {
    emplace_front(MoperSTL::forward<Args>(args)...);
    return begin_;
  }
  else if (pos.cur == end_.cur)
  {
    emplace_back(MoperSTL::forward<Args>(args)...);
    return end_ - 1;
  }
  return insert_aux(pos, MoperSTL::forward<Args>(args)...);
}
...
deque<T>::insert(iterator position, value_type&& value)
{
  if (position.cur == begin_.cur)
  {
    emplace_front(MoperSTL::move(value));
    return begin_;
  }
  else if (position.cur == end_.cur)
  {
  ...
  {
    return insert_aux(position, MoperSTL::move(value));
  }
}
...
void deque<T>::insert(iterator position, size_type n, const value_type& value)
{
  if (position.cur == begin_.cur)
  {
    ...
    MoperSTL::uninitialized_fill_n(new_begin, n, value);
    begin_ = new_begin;
  }
  else if (position.cur == end_.cur)
  {
    ...
    auto new_end = end_ + n;
    MoperSTL::uninitialized_fill_n(end_, n, value);
    end_ = new_end;
  }
  else
  {
...
deque<T>::erase(iterator position)
{
  ...
  {
    MoperSTL::copy_backward(begin_, position, next);
    pop_front();
  }
  else
  {
    MoperSTL::copy(next, end_, position);
    pop_back();
  }
...
deque<T>::erase(iterator first, iterator last)
{
  ...
  else
  {
    ...
    {
      MoperSTL::copy_backward(begin_, first, last);
...
void deque<T>::clear()
{
  ...
  { // 有两个以上的缓冲区
      MoperSTL::destory(begin_.cur, begin_.last);
      MoperSTL::destory(end_.first, end_.cur);
  }
  else
  {
      MoperSTL::destory(begin_.cur, end_.cur);
  }
  shrink_to_fit();
  end_ = begin_;
}
...
void deque<T>::swap(deque& rhs) noexcept
{
  if (this != &rhs)
  {
    MoperSTL::swap(begin_, rhs.begin_);
    MoperSTL::swap(end_, rhs.end_);
    MoperSTL::swap(map_, rhs.map_);
    MoperSTL::swap(map_size_, rhs.map_size_);
  }
}
...
map_init(size_type nElem)
{
  ...
  map_size_ = MoperSTL::max(static_cast<size_type>(DEQUE_MAP_INIT_SIZE), nNode + 2);
  try
  {
...
fill_init(size_type n, const value_type& value)
{
  ...
  if (n != 0)
  {
    ...
    {
      MoperSTL::uninitialized_fill(*cur, *cur + buffer_size, value);
    }
    MoperSTL::uninitialized_fill(end_.first, end_.cur, value);
  }
}
...
{
  const size_type n = MoperSTL::distance(first, last);
  map_init(n);
...
copy_init(FIter first, FIter last, forward_iterator_tag)
{
  ...
  {
    auto next = first;
    MoperSTL::advance(next, buffer_size);
    MoperSTL::uninitialized_copy(first, next, *cur);
    first = next;
  }
  MoperSTL::uninitialized_copy(first, last, end_.first);
}
...
{  
  ...
  const size_type len2 = MoperSTL::distance(first, last);
  if (len1 < len2)
  {
    auto next = first;
    MoperSTL::advance(next, len1);
    MoperSTL::copy(first, next, begin_);
    insert_dispatch(end_, next, last, forward_iterator_tag{});
  }
  else
  {
    erase(MoperSTL::copy(first, last, begin_), end_);
  }
}
...
{
  const size_type elems_before = position - begin_;
  value_type value_copy = value_type(MoperSTL::forward<Args>(args)...);
  if (elems_before < (size() / 2))
  { // 在前半段插入
    emplace_front(front());
    auto front1 = begin_;
    ...
    auto pos = position;
    ++pos;
    MoperSTL::copy(front2, pos, front1);
  }
  else
  { // 在后半段插入
    emplace_back(back());
    ...
    --back2;
    position = begin_ + elems_before;
    MoperSTL::copy_backward(position, back2, back1);
  }
  *position = MoperSTL::move(value_copy);
  return position;
}
...
fill_insert(iterator position, size_type n, const value_type& value)
{
  ...
  if (elems_before < (len / 2))
  {
    ...
    try
    {
      if (elems_before >= n)
      {
        ...
        MoperSTL::uninitialized_copy(begin_, begin_n, new_begin);
        begin_ = new_begin;
        MoperSTL::copy(begin_n, position, old_begin);
        MoperSTL::fill(position - n, position, value_copy);
      }
      else
      {
        MoperSTL::uninitialized_fill(
          MoperSTL::uninitialized_copy(begin_, position, new_begin), begin_, value_copy);
        begin_ = new_begin;
        MoperSTL::fill(old_begin, position, value_copy);
      }
    }
    catch (...)
    {
  ...
  else
  {
    ...
    try
    {
      ...
      {
        auto end_n = end_ - n;
        MoperSTL::uninitialized_copy(end_n, end_, end_);
        end_ = new_end;
        MoperSTL::copy_backward(position, end_n, old_end);
        MoperSTL::fill(position, position + n, value_copy);
      }
      else
      {
        MoperSTL::uninitialized_fill(end_, position + n, value_copy);
        MoperSTL::uninitialized_copy(position, end_, position + n);
        end_ = new_end;
        MoperSTL::fill(position, old_end, value_copy);
      }
    }
    catch (...)
    {
  ...
  if (elems_before < (len / 2))
  {
    ...
    try
    {
      if (elems_before >= n)
      {
        ...
        begin_ = new_begin;
        MoperSTL::copy(begin_n, position, old_begin);
        MoperSTL::copy(first, last, position - n);
      }
      else
      {
        auto mid = first;
        MoperSTL::advance(mid, n - elems_before);
        MoperSTL::uninitialized_copy(first, mid,
                                  MoperSTL::uninitialized_copy(begin_, position, new_begin));
        begin_ = new_begin;
        MoperSTL::copy(mid, last, old_begin);
      }
    }
    catch (...)
    {
  ...
  else
  {
    ...
    try
    {
      if (elems_after > n)
      {
        ...
        end_ = new_end;
        MoperSTL::copy_backward(position, end_n, old_end);
        MoperSTL::copy(first, last, position);
      }
      else
      {
        auto mid = first;
        MoperSTL::advance(mid, elems_after);
        MoperSTL::uninitialized_copy(position, end_,
                                  MoperSTL::uninitialized_copy(mid, last, end_));
        end_ = new_end;
        MoperSTL::copy(first, mid, position);
      }
    }
    catch (...)
    {
...
insert_dispatch(iterator position, FIter first, FIter last, forward_iterator_tag)
{
  ...
  if (position.cur == begin_.cur)
  {
    ...
    try
    {
      MoperSTL::uninitialized_copy(first, last, new_begin);
      begin_ = new_begin;
    }
    catch (...)
    {
  ...
  else if (position.cur == end_.cur)
  {
    ...
    try
    {
      MoperSTL::uninitialized_copy(first, last, end_);
      end_ = new_end;
    }
    catch (...)
    {
...
{
  const size_type new_map_size = MoperSTL::max(map_size_ << 1,
...
bool operator==(const deque<T>& lhs, const deque<T>& rhs)
{
  return lhs.size() == rhs.size() && 
    MoperSTL::equal(lhs.begin(), lhs.end(), rhs.begin());
}

template <class T>
bool operator<(const deque<T>& lhs, const deque<T>& rhs)
{
  return MoperSTL::lexicographical_compare(
