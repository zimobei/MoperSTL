#ifndef MOPER_STL_UNORDERED_MAP_H
#define MOPER_STL_UNORDERED_MAP_H
...
// 参数一代表键值类型，参数二代表实值类型，参数三代表哈希函数，缺省使用 MoperSTL::hash
// 参数四代表键值比较方式，缺省使用 MoperSTL::equal_to
template <class Key, class T, class Hash = MoperSTL::hash<Key>, class KeyEqual = MoperSTL::equal_to<Key>>
class unordered_map {
private:
  // 使用 hashtable 作为底层机制
    typedef hashtable<MoperSTL::pair<const Key, T>, Hash, KeyEqual> base_type;
    base_type ht_;
...
public:
  // 构造、复制、移动、析构函数
    ...
    unordered_map(InputIterator first, InputIterator last,
                  ...
                  const Hash& hash = Hash(),
                  const KeyEqual& equal = KeyEqual())
        : ht_(MoperSTL::max(bucket_count, static_cast<size_type>(MoperSTL::distance(first, last))), hash, equal) {
    ...
    unordered_map(std::initializer_list<value_type> ilist,
                  ...
                  const Hash& hash = Hash(),
                  const KeyEqual& equal = KeyEqual())
        :ht_(MoperSTL::max(bucket_count, static_cast<size_type>(ilist.size())), hash, equal) {
...
// 容量相关
    ...
    pair<iterator, bool> emplace(Args&& ...args) {
        return ht_.emplace_unique(MoperSTL::forward<Args>(args)...);
    }

    template <class ...Args>
    iterator emplace_hint(const_iterator hint, Args&& ...args) {
        return ht_.emplace_unique_use_hint(hint, MoperSTL::forward<Args>(args)...);
    }
...
// insert
    ...
    pair<iterator, bool> insert(value_type&& value) {
        return ht_.emplace_unique(MoperSTL::move(value));
    }
    ...
    iterator insert(const_iterator hint, value_type&& value) {
        return ht_.emplace_unique_use_hint(hint, MoperSTL::move(value));
    }
...
// 查找相关
    ...
    mapped_type& operator[](key_type&& key) {
        iterator it = ht_.find(key);
        if (it.node == nullptr)
            it = ht_.emplace_unique(MoperSTL::move(key), T{}).first;
        return it->second;
    }
...
// 容量相关
    ...
    iterator emplace(Args&& ...args) {
        return ht_.emplace_multi(MoperSTL::forward<Args>(args)...);
    }

    template <class ...Args>
    iterator emplace_hint(const_iterator hint, Args&& ...args) {
        return ht_.emplace_multi_use_hint(hint, MoperSTL::forward<Args>(args)...);
    }
...
// insert
    ...
    iterator insert(value_type&& value) {
        return ht_.emplace_multi(MoperSTL::move(value));
    }
    ...
    iterator insert(const_iterator hint, value_type&& value) {
        return ht_.emplace_multi_use_hint(hint, MoperSTL::move(value));
    }
...
#endif /* MOPER_STL_UNORDERED_MAP_H */
